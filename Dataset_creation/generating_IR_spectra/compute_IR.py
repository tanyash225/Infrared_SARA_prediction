'''Wrapper for xTB computing IR spectra for the given SMILES string'''
...
def get_confs(smiles):
    '''Generates the most stable conformer for the given molecule'''
    mol = Chem.MolFromSmiles(smiles)
    cgen = ConformerGenerator(
        max_conformers=1,
        rmsd_threshold=0.5,
        force_field='uff',
        pool_multiplier=25
...
def get_parser() -> argparse.ArgumentParser:
    '''Returns parser object for the script'''
    parser = argparse.ArgumentParser(
        description = 'Computes IR spectra for the given SMILES string using RDKit ' \
                      'for conformer generation and xTB for computations'
    )
    parser.add_argument('smiles', type=str, help='SMILES of the input molecule')
    parser.add_argument('--nproc', type=int, default=1,
                        help='Number of parallel processes for xTB')
    
    return parser


def get_ir_spectrum(smiles: str, nproc: int = 1) -> dict:
    '''Computes IR spectrum at GFN2-xTB level'''
    # get mol confs
    mol = get_confs(smiles)
    # run xTB
    with tempfile.TemporaryDirectory() as tmpdir:
        Chem.MolToMolFile(mol, os.path.join(tmpdir, 'x.mol'))
        res = subprocess.run(['xtb', 'x.mol', '--gfn2', '--ohess', '-P', f'{nproc}'],
                             cwd=tmpdir, capture_output=True, text=True)
        with open(os.path.join(tmpdir, 'vibspectrum'), 'rt') as inpf:
            spec = inpf.read()
    # extract spectrum
    spec = [l.strip() for l in spec.split('\n') if l.strip()][9:-1]
    spec = [l.split() for l in spec if l.split()[-1].upper() == 'YES']
    spec = {float(l[2]): float(l[3]) for l in spec}
...
if __name__ == '__main__':
    ...
    # get conformers
    spec = get_ir_spectrum(args.smiles, args.nproc)
    
    # print spectrum
    writer = csv.writer(sys.stdout)
    writer.writerow(['Index', 'Frequency (cm-1)', 'Intensity (km/mol)'])
    for i, (freq, intens) in enumerate(spec.items()):
        writer.writerow([i, freq, intens])
